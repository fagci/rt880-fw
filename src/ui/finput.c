...
void FINPUT_render(void) {
  ...
  PrintfEx(LCD_WIDTH - 3, BASE_Y, POS_R, C_WHITE, C_BLACK, F_LG, "%s",
           displayStr);
  PrintfEx(LCD_WIDTH - 3, BASE_Y + 8, POS_R, C_WHITE, C_BLACK, F_NORM, "%s",
           unitSuffix);
  ...
  if (currentConfig.is_range) {
    ...
    Printf(0, 16, rangeStr);
  }

  if (currentConfig.min) {
    switch (currentConfig.unit) {
    case UNIT_MHZ:
      Printf(0, 32, "Min: %u.%05u MHz", currentConfig.min / 100000,
             currentConfig.min % 100000);
      break;
    case UNIT_KHZ:
      Printf(0, 32, "Min: %u.%02u kHz", currentConfig.min / 100,
             currentConfig.min % 100);
      break;
    case UNIT_HZ:
      Printf(0, 32, "Min: %u Hz", currentConfig.min);
      break;
    case UNIT_VOLTS:
      Printf(0, 32, "Min: %u.%02u V", currentConfig.min / 100,
             currentConfig.min % 100);
      break;
    default:
      Printf(0, 32, "Min: %u", currentConfig.min);
      break;
    }
  }
  ...
  if (currentConfig.max) {
    switch (currentConfig.unit) {
    case UNIT_MHZ:
      Printf(0, 42, "Max: %u.%05u MHz", currentConfig.max / 100000,
             currentConfig.max % 100000);
      break;
    case UNIT_KHZ:
      Printf(0, 42, "Max: %u.%02u kHz", currentConfig.max / 100,
             currentConfig.max % 100);
      break;
    case UNIT_HZ:
      Printf(0, 42, "Max: %u Hz", currentConfig.max);
      break;
    case UNIT_VOLTS:
      Printf(0, 42, "Max: %u.%02u V", currentConfig.max / 100,
             currentConfig.max % 100);
      break;
    default:
      Printf(0, 42, "Max: %u", currentConfig.max);
      break;
    }
  }
}
