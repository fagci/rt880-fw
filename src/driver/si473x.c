...
/* ─── SI4732 Patch data (8840 bytes) ───────────────────────────── */
static const uint8_t si4732_patch[] = {
...
static const uint32_t si4732_patch_len = 8840;

/* ─── Patch download ────────────────────────────────────────────── */
static void SI47XX_downloadPatch(void) {
  uint32_t i = 0;
  while (i < si4732_patch_len) {
    /* Wait for CTS */
    if (!waitToSend())
      return;

    /* Find next PATCH_ARGS (0x15) or PATCH_DATA (0x16) command */
    uint8_t cmd = si4732_patch[i];
    uint8_t buf[9];
    buf[0] = cmd;

    if (cmd == 0x15) {
      /* PATCH_ARGS: 8 bytes total */
      memcpy(buf + 1, si4732_patch + i + 1, 7);
      SI47XX_WriteBuffer(buf, 8);
      delay_ms(10);
      i += 8;
    } else {
      /* PATCH_DATA: 8 bytes total */
      memcpy(buf + 1, si4732_patch + i + 1, 7);
      SI47XX_WriteBuffer(buf, 8);
      delay_ms(10);
      i += 8;
